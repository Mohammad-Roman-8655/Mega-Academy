...
function MockPapers() {
  ...
  const [selectedStandard, setSelectedStandard] = useState(""); // State to store selected class
    ...
    const fetchMockPapers = async (standard = "") => {
      try {
        const url = standard
          ? `http://localhost:8080/api/MockPaper?standard=${standard}`
          : "";
 ...
 <option value="Standards" className='p-2'>Select Class </option>
    ...
    <div className="row-1 flex  justify-center mt-[10%]  w-[100%] mb-[10%]">
      <button onClick={() => fetchMockPapers(selectedStandard)} className='lg:w-[30%] md:w-[30%] sm:w-[80%] w-[80%] lg:text-xl md:text-xl sm:text-lg text-lg text-white font-bold lg:p-3 md:p-3 sm:p-1 p-1 border-2 rounded-md bg-blue-500 border-blue-950 hover:border-black '>Search</button>
    </div>
    </div>
      ...
      <div className='individual-pdfs '>
        <table className='border-collapse border border-black  mx-auto lg:w-[85%] md:w-[85%] sm:w-[98%] w-[98%] text-center shadow-3xl'>
         ...
         <caption className='border border-black lg:p-5 md:p-5 sm:p-2 p-2 lg:text-2xl md:text-2xl sm:text-xl text-xl font-bold bg-blue-900 text-white'>Mock Papers List</caption>
            ...
            <tbody>
          {
        MockPapers.map((MockPaper,idx)=>{
          return(
            <tr key={idx+1}>
              <td className='border border-black lg:p-5 md:p-5 sm:p-2 p-2 lg:text-lg md:text-lg sm:text-md text-md '>{idx+1}</td>
              <td className='border border-black lg:p-5 md:p-5 sm:p-2 p-2 lg:text-lg md:text-lg sm:text-md text-md '>{MockPaper.subjectName}</td>
              <td className='border border-black lg:p-5 md:p-5 sm:p-2 p-2  font-bold '><a className="lg:text-lg md:text-lg sm:text-md text-md border-2 bg-blue-500 hover:border-black text-white lg:p-3 md:p-3 sm:p-1 p-1 rounded-lg" href={MockPaper.mockPaperUrl}>Download</a></td>
            </tr>
          )
        })
      }
